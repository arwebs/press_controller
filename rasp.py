...
DO  = 23
MOSI = 24
sensor = MAX31855.MAX31855(CLK, CS1, DO)
sensor2 = MAX31855.MAX31855(CLK, CS2, DO)
mcp = Adafruit_MCP3008.MCP3008(clk=CLK, cs=CSAnalog, miso=DO, mosi=MOSI)
while True:
    temp = sensor.readTempC()
    internal = sensor.readInternalC()
    print('Thermocouple 1 Temperature: {0:0.3F}*C / {1:0.3F}*F'.format(temp, c_to_f(temp)))
    print('    Internal 1 Temperature: {0:0.3F}*C / {1:0.3F}*F'.format(internal, c_to_f(internal)))
    # time.sleep(1.0)
    temp2 = sensor2.readTempC()
    internal2 = sensor2.readInternalC()
    print('Thermocouple 2 Temperature: {0:0.3F}*C / {1:0.3F}*F'.format(temp2, c_to_f(temp2)))
    print('    Internal 2 Temperature: {0:0.3F}*C / {1:0.3F}*F'.format(internal2, c_to_f(internal2)))
    values = [0] * 8
    for i in range(8):
    ...
    lcd.clear()
    lcd.message('T1:{0:0.2F}*C/{1:0.2F}*F\nT2:{2:0.2F}*C/{3:0.2F}*F\nPOT:{4:0.2F}'.format(internal, c_to_f(internal),internal2, c_to_f(internal2), values[0]))
    time.sleep(2.0)
